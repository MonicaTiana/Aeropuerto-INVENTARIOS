...
{
    public partial class BorrarPapeleria : Form
    {
        ...
        public BorrarPapeleria()
        {
            InitializeComponent();
        }

        private void BorrarPapeleria_Load(object sender, EventArgs e)
        {
        ...
        {
            conexion.Open();
            string consulta = "delete from Papeleria where id_codigo ='" + id + "'";
        ...
        {
            conexion.Open();
            string consulta = "delete from Papeleria where NOMBRE ='" + cm + "'";
            comando = new SqlCommand(consulta, conexion);
            if (comando.ExecuteNonQuery() > 0)
            {
                return true;
        ...
        {
            string consulta = "Select id_codigo,NOMBRE  from Papeleria";
            comando = new SqlCommand(consulta, conexion);
        ...
        private void btn_eliminar_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                conexion.Open();
                string consulta = "delete from Papeleria where id_codigo='" + id_codigo + "'";
                comando = new SqlCommand(consulta, conexion);
                SqlDataAdapter adaprar = new SqlDataAdapter(comando);
                adaprar.Fill(table, "Papeleria");
                Cargar_Data();
