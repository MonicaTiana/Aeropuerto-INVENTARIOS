...
{
    public partial class Modificar_Registro_Limpieza : Form
    {
        ...
        public Modificar_Registro_Limpieza()
        {
            InitializeComponent();
        }
        ...
        private void Modificar_Registro_Limpieza_Load(object sender, EventArgs e)
        {
            deshabilitar();
            ...
            DataTable dato5 = cargar_comoarea();
        ...
        {
            conexion.Open();
            string consulta = "UPDATE Limpiesa set  NOMBRE=@NOMBRE, CANTIDAD=@Cantidad,PRECIO=@Precio, AREA=@Area,DEPARTAMENTO=@Departamento, MARCA=@Marca,MODELO=@Modelo,TIPO_INVENTARIO=@TI,TIPO=@Tipo,FECHA=@Fecha where id_codigo=" + Codigo;
        ...
        {
            string consulta = "Select distinct Tipo_Inventario from Limpiesa";
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptar = new SqlDataAdapter(comando);
            DataTable tabla = new DataTable();
            adaptar.Fill(tabla);
        ...
        {
            string consulta = "Select distinct TIPO from Limpiesa";
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptar = new SqlDataAdapter(comando);
            DataTable tabla = new DataTable();
            adaptar.Fill(tabla);
        ...
        private void buscar_codigo(int codigo)
        {
            ...
            string consulta = "select * from Limpiesa where id_codigo=" + codigo;
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptar = new SqlDataAdapter(comando);
            adaptar.Fill(tabla, "Limpiesa");

            lb_Codigo.Text = tabla.Tables["Limpiesa"].Rows[0][0].ToString();
            txtnombre.Text = tabla.Tables["Limpiesa"].Rows[0][1].ToString();
            txtcantidad.Text = tabla.Tables["Limpiesa"].Rows[0][2].ToString();
            txtprecio.Text = tabla.Tables["Limpiesa"].Rows[0][3].ToString();
            comboBoxarea.Text = tabla.Tables["Limpiesa"].Rows[0][4].ToString();
            comboBoxdepa.Text = tabla.Tables["Limpiesa"].Rows[0][5].ToString();
            txtmarca.Text = tabla.Tables["Limpiesa"].Rows[0][6].ToString();
            txtmodelo.Text = tabla.Tables["Limpiesa"].Rows[0][7].ToString();
            comboBoxtipoinv.Text = tabla.Tables["Limpiesa"].Rows[0][8].ToString();
            comboBoxtipo.Text = tabla.Tables["Limpiesa"].Rows[0][9].ToString();
            txt_fecha.Text = tabla.Tables["Limpiesa"].Rows[0][10].ToString();
            conexion.Close();
        }
