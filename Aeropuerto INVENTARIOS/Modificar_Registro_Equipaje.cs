...
{
    public partial class Modificar_Registro_Equipaje : Form
    {
        ...
        public Modificar_Registro_Equipaje()
        {
            InitializeComponent();
        ...
        {
            Registro_Equipaje registro = new Registro_Equipaje();
            this.Close();
            registro.Show();
        }

        private void Modificar_Registro_Equipaje_Load(object sender, EventArgs e)
        {
            Deshabilitar();
        ...
        {
            string consulta = "select TIPO_INVENTARIO from Equipaje";
        ...
        private void btn_buscar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Dato incorrecto", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        private DataTable Cargar_area()
        {
            string consulta = "select AREA from Equipaje";
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptador = new SqlDataAdapter(comando);
            DataTable tabla = new DataTable();
            adaptador.Fill(tabla);
        ...
        {
            string consulta = "select DEPARTAMENTO from Equipaje";
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptador = new SqlDataAdapter(comando);
            DataTable tabla = new DataTable();
            adaptador.Fill(tabla);
        ...
        {
            string consulta = "select TIPO from Equipaje";
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptador = new SqlDataAdapter(comando);
            DataTable tabla = new DataTable();
            adaptador.Fill(tabla);
        ...
        public void Actualizar(int id)
        {
            ...
            string consulta = "update Equipaje set NOMBRE = @NOMBRE, CANTIDAD = @CANTIDAD, PRECIO = @PRECIO, AREA = @AREA, DEPARTAMENTO = @DEPARTAMENTO, MARCA = @MARCA, MODELO = @MODELO, TIPO_INVENTARIO = @TIPO_INV, TIPO = @TIPO, FECHA = @FECHA where id_codigo =  " + id;
            comando = new SqlCommand(consulta, conexion);
        ...
        public void BuscarID(int id)
        {
            ...
            string consulta = "select * from Equipaje where id_codigo = " + id;
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptador = new SqlDataAdapter(comando);
            adaptador.Fill(tabla, "Equipaje");
            lbl_codigo.Text = tabla.Tables["Equipaje"].Rows[0][0].ToString();
            txtnombre.Text = tabla.Tables["Equipaje"].Rows[0][1].ToString();
            txtcantidad.Text = tabla.Tables["Equipaje"].Rows[0][2].ToString();
            txtprecio.Text = tabla.Tables["Equipaje"].Rows[0][3].ToString();
            comboBoxarea.Text = tabla.Tables["Equipaje"].Rows[0][4].ToString();
            comboBoxdepa.Text = tabla.Tables["Equipaje"].Rows[0][5].ToString();
            txtmarca.Text = tabla.Tables["Equipaje"].Rows[0][6].ToString();
            txtmodelo.Text = tabla.Tables["Equipaje"].Rows[0][7].ToString();
            comboBoxtipoinv.Text = tabla.Tables["Equipaje"].Rows[0][8].ToString();
            comboBoxtipo.Text = tabla.Tables["Equipaje"].Rows[0][9].ToString();
            txt_fecha.Text = tabla.Tables["Equipaje"].Rows[0][10].ToString();
            conexion.Close();
        }
        ...
        private void txtprecio_HideSelectionChanged(object sender, EventArgs e)
        {
