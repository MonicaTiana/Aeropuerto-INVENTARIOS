...
{
    public partial class Borrar_Registro_equipaje : Form
    {
        SqlCommand comando;
        int contador = 0;
        private void Borrar_Registro_equipaje_Load(object sender, EventArgs e)
        {
        ...
        public Borrar_Registro_equipaje()
        {
            InitializeComponent();
        }
        ...
        {
            conexion.Open();
            string consulta = "delete from Equipaje where id_codigo = " + id;
            comando = new SqlCommand(consulta, conexion);
            comando.ExecuteNonQuery();
            contador++;
            conexion.Close();
        }
        private void Eliminar_nom(string nom)
        {
            conexion.Open();
            string consulta = "delete from Equipaje where NOMBRE = " + nom;
            comando = new SqlCommand(consulta, conexion);
            comando.ExecuteNonQuery();
            contador++;
            conexion.Close();
        ...
        private void Cargar_datagrid()
        {
            ...
            string consulta = "select id_codigo, NOMBRE, CANTIDAD, PRECIO, AREA, DEPARTAMENTO, MARCA, MODELO, TIPO_INVENTARIO, TIPO, FECHA from Equipaje";
            comando = new SqlCommand(consulta, conexion);
        ...
        {
            Registro_Equipaje registro_e = new Registro_Equipaje();
            registro_e.Show();
            this.Close();
        }
