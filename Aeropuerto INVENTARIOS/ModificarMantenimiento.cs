...
{
    public partial class ModificarMantenimiento : Form
    {
        ...
        private void buscar_codigo(int codigo)
        {
            ...
            string consulta = "select * from Mantenimento where id_codigo=" + codigo;
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptar = new SqlDataAdapter(comando);
            adaptar.Fill(tabla, "Mantenimento");

            lb_Codigo.Text = tabla.Tables["Mantenimento"].Rows[0][0].ToString();
            txtnombre.Text = tabla.Tables["Mantenimento"].Rows[0][1].ToString();
            txtcantidad.Text = tabla.Tables["Mantenimento"].Rows[0][2].ToString();
            txtprecio.Text = tabla.Tables["Mantenimento"].Rows[0][3].ToString();
            comboBoxarea.Text = tabla.Tables["Mantenimento"].Rows[0][4].ToString();
            comboBoxdepa.Text = tabla.Tables["Mantenimento"].Rows[0][5].ToString();
            txtmarca.Text = tabla.Tables["Mantenimento"].Rows[0][6].ToString();
            txtmodelo.Text = tabla.Tables["Mantenimento"].Rows[0][7].ToString();
            comboBoxtipoinv.Text = tabla.Tables["Mantenimento"].Rows[0][8].ToString();
            comboBoxtipo.Text = tabla.Tables["Mantenimento"].Rows[0][9].ToString();
            txt_fecha.Text = tabla.Tables["Mantenimento"].Rows[0][10].ToString();
            conexion.Close();
        }
        private void actualizar(int Codigo)
        {
            conexion.Open();
            string consulta = "UPDATE Mantenimento set  NOMBRE=@NOMBRE, CANTIDAD=@Cantidad,PRECIO=@Precio, AREA=@Area,DEPARTAMENTO=@Departamento, MARCA=@Marca,MODELO=@Modelo,TIPO_INVENTARIO=@TI,TIPO=@Tipo,FECHA=@Fecha where id_codigo=" + Codigo;
            comando = new SqlCommand(consulta, conexion);
        ...
        {
            string consulta = "Select distinct Tipo_Inventario from Mantenimento";
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptar = new SqlDataAdapter(comando);
            DataTable tabla = new DataTable();
            adaptar.Fill(tabla);
        ...
        {
            string consulta = "Select distinct TIPO from Mantenimento";
            comando = new SqlCommand(consulta, conexion);
            SqlDataAdapter adaptar = new SqlDataAdapter(comando);
            DataTable tabla = new DataTable();
            adaptar.Fill(tabla);
        ...
        private void ModificarMantenimiento_Load(object sender, EventArgs e)
        {
            deshabilitar();
